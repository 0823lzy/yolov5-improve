...
from utils.general import check_img_size, check_requirements, check_imshow, colorstr, non_max_suppression, \
    apply_classifier, scale_boxes, xyxy2xywh, strip_optimizer, set_logging, increment_path
...
class DetThread(QThread):
    ...
    def run(self,
        ...
        try:
            ...
            # Load model
            model = DetectMultiBackend(self.weights, device=device)  # load FP32 model
            num_params = 0
            ...
            while True:
                ...
                if self.current_weight != self.weights:
                    ...
                    for param in model.parameters():
                        num_params += param.numel()
                    stride, names = model.stride, model.names  # model stride
                    imgsz = check_img_size(imgsz, s=stride)  # check image size
                    if half:
                        model.half()  # to FP16
                    # Run inference
